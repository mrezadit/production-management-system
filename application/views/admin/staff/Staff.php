    ...
    <div class="card-header p-0 w-75 position-fixed mt-n4 mx-2 z-index-2">
        <div class="shadow-dark border-radius-lg d-flex px-5 pt-4 pb-3">
            ...
            <div class="col-4 text-end">
                <a href="<?= site_url('admin/staff/addstaff'); ?>" class="btn bg-gradient-dark mb-0">Add Staff</a>
            </div>
        </div>
    </div>
</div>
            ...
            <div class="table-responsive p-0">
                <table id="table-data" class="table align-items-center justify-content-center mb-0">
                    <thead>
                        ...
                        <th class="text-uppercase text-secondary text-xs font-weight-bolder opacity-7">Staff Name</th>
                    ...
                    <?php if (!empty($staff)) : $i = 1; foreach ($staff as $value) : ?>
                        ...
                        <?php if ( $value->st_status == 1) :?>
                            <span class="text-xs font-weight-bold text-info">Ready</span>
                        <?php elseif ( $value->st_status == 2) : ?>
                            <span class="text-xs font-weight-bold text-warning">Scheduled</span>
                        </td>
                        <?php endif ;?> 
                        <td class="pl-4">
                            <a href="<?= site_url('admin/staff/'.$value->id_staff.'/update'); ?>" rel="tooltip" title="Edit Task" class="btn btn-info btn-link btn-sm">
                                <i class="material-icons">edit</i>
                            </a>
                            <a href="<?= site_url('admin/deleteStaff/'.$value->id_staff); ?>" rel="tooltip" title="Remove" class="btn btn-danger btn-link btn-sm">
